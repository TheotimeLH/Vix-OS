...
static const double NOTES[NUM_OCTAVES * OCTAVE_SIZE] = {
    // O1
    32.703195662574764,
    34.647828872108946,
    36.708095989675876,
...
static uint64 sample = 0;

static uint8 volume_master;
static uint8 volumes[NUM_NOTES];
static uint8 notes[NUM_NOTES];
static uint8 waves[NUM_NOTES];
...
static void fill(short *buf, size_t len) {
    for (size_t i = 0; i < len; i++) {
        double f = 0.0;

        for (size_t j = 0; j < NUM_NOTES; j++) {
            uint8 octave = (notes[j] >> 4) & 0xF,
               note = notes[j] & 0xF;
        ...
        buf[i] = (short) (((volume_master / 255.0) * 4096.0) * f);
...
static void dsp_write(uint8 b) {
    while (inb(DSP_WRITE) & 0x80);
    outportb(DSP_WRITE, b);
}

static void dsp_read(uint8 b) {
...
static void reset() {
    ...
    uint8 status = inb(DSP_READ_STATUS);
    if (~status & 128) {
        goto fail;
    }
    ...
    return;
fail:
		print_string("ERREUR CRITIQUE INITIALISATION DSP");
		
}

static void set_sample_rate(uint16 hz) {
    dsp_write(DSP_SET_RATE);
    dsp_write((uint8) ((hz >> 8) & 0xFF));
    dsp_write((uint8) (hz & 0xFF));
}

static void transfer(void *buf, uint32 len) {
    uint8 mode = 0x48;
    ...
    outportb(DMA_BASE_ADDR, (uint8) ((offset >> 0) & 0xFF));
    outportb(DMA_BASE_ADDR, (uint8) ((offset >> 8) & 0xFF));

    // write transfer length
    outportb(DMA_COUNT, (uint8) (((len - 1) >> 0) & 0xFF));
    outportb(DMA_COUNT, (uint8) (((len - 1) >> 8) & 0xFF));

    // write buffer
    outportb(0x8B, ((uint32) buf) >> 16);

    // enable DMA channel
    outportb(0xD4, DMA_CHANNEL_16 % 4);
}

static void sb16_irq_handler(registers_t regs) {
...
static void configure() {
    register_interrupt_handler(MIXER_IRQ, sb16_irq_handler);
    outb(DSP_IRQ, DSP_MIXER);
    outb(MIXER_IRQ_DATA, DSP_MIXER_DATA);

    uint8 v = MIXER_IRQ;
    if (v != MIXER_IRQ) {
        print_string( "SB16 HAS INCORRECT IRQ: ");
    }
}
...
void sound_init() {
    ...
    uint16 sample_count = (BUFFER_SIZE / 2) - 1;
    dsp_write(DSP_PLAY | DSP_PROG_16 | DSP_AUTO_INIT);
    dsp_write(DSP_SIGNED | DSP_MONO);
    dsp_write((uint8) ((sample_count >> 0) & 0xFF));
    dsp_write((uint8) ((sample_count >> 8) & 0xFF));
